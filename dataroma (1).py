...
# Dataroma tarih formatları: "Q3 2024", "Nov 2024", "11/2024", "2024-11-15"
DATE_RE = re.compile(
    r"\b(Q[1-4]\s*20\d{2}"           # Q3 2024
    r"|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s*20\d{2}"  # Nov 2024
    r"|\d{1,2}/20\d{2}"              # 11/2024
    r"|20\d{2}-\d{2}-\d{2}"          # 2024-11-15
    r"|20\d{2}/\d{2}/\d{2})\b",      # 2024/11/15
    re.IGNORECASE
)

def _find_table(soup):
    """Yönetici tablosunu birden fazla stratejiyle bul."""
    # 1. id="grid" (klasik)
    t = soup.find("table", id="grid")
    if t: return t
    # 2. id içinde "grid" geçen
    for t in soup.find_all("table"):
        if "grid" in (t.get("id") or ""):
            return t
    # 3. thead'inde "Manager" ya da "Portfolio" geçen tablo
    for t in soup.find_all("table"):
        header = t.find("thead") or t.find("tr")
        if header and re.search(r"manager|portfolio|fund", header.get_text(), re.I):
            return t
    # 4. En çok satırlı, <a href> içeren tablo
    candidates = [(len(t.find_all("tr")), t) for t in soup.find_all("table") if t.find("a")]
    if candidates:
        return max(candidates, key=lambda x: x[0])[1]
    return None

def _extract_date(tds):
    """td listesinden tarih string'ini çıkar. Önce son td'ye bak."""
    # Önce sondan başa tara (tarih genellikle son sütunda)
...
def parse_managers(html):
    ...
    for row in grid.find_all("tr"):
        ...
        updated   = _extract_date(tds)
        port_val  = tds[1].get_text(strip=True) if len(tds) > 1 else ""
...
<div id="banner" class="wait" style="display:none"></div>
...
async function checkBackend(){
  try{
    await api('/api/status',{timeout:5000});
    $('sdot').className='dot live';$('stxt').textContent='Bağlı';
    $('banner').style.display='none';
    loadManagers();
  }catch(e){
    $('sdot').className='dot err';$('stxt').textContent='Bağlantı yok';
    setBanner('fail','❌','Sunucuya bağlanılamadı.');
    $('mgList').innerHTML='<div class="list-empty">⚠ Bağlantı hatası</div>';
  }
}
function setBanner(cls,icon,html){
  const b=$('banner');b.style.display='flex';
  b.className=cls;
  b.innerHTML=`<span class="bico">${icon}</span><div class="btxt">${html}</div><button class="btn" onclick="checkBackend()">↻ Tekrar dene</button>`;
}
...
async function loadManagers(force=false){
  ...
  try{
    ...
    $('stxt').textContent=`${allManagers.length} yönetici`;
    applyFilters();
